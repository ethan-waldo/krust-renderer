...
pub fn ray_color(
    ...
    if let (true, Some(hit_rec)) = world.hit(&r, 0.0001, INF) {
        if let Some((ray, albedo, emission, lobe)) =
            hit_rec.material.scatter(&r, &hit_rec, quad_lights)
        {
            // sample scene
            ...
            let emit = if hit_rec.front_face {
                emission
            } else {
                Color::black()
            };
            ...
            color.diffuse = if lobe == "diffuse" {
                composite
            } else {
                Color::black()
            };
            color.specular = if lobe == "specular" {
                composite
            } else {
                Color::black()
            };
...
pub fn first_hit_aovs(r: &Ray, world: &Object) -> Lobes {
    if let (true, Some(hit_rec)) = world.hit(&r, 0.0001, INF) {
        let sample = hit_rec.material.surface_sample(&hit_rec);
        let roughness = sample.roughness;
        return Lobes {
            emission: sample.emission,
            normal: Color::new(hit_rec.normal.x, hit_rec.normal.y, hit_rec.normal.z, 1.0),
            albedo: sample.albedo,
            roughness: Color::new(roughness, roughness, roughness, 1.0),
            depth: Color::new(hit_rec.t, hit_rec.t, hit_rec.t, 1.0),
            position: Color::new(hit_rec.point.x, hit_rec.point.y, hit_rec.point.z, 1.0),
            ..Lobes::empty()
        };
    }
...
pub fn render_chunk(
    ...
    for pixel in pixel_chunks {
        ...
        .with_auxiliary(first_hit_aovs(&r, bvh));
